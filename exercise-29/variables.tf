...
variable "ec2_instance_config_map" {
  ...
  # validation to ensure only t3.micro used
  validation {
    condition = alltrue([
  ...
  #  validation to ensure only ubuntu and nginx images used
  validation {
...
#  variable defining the configuration for our subnets 
variable "subnet_config" {
  type = map(object({
    cidr_block = string
  }))

  # check every subnet in map, take its cidr_block and check whether it's valid
  validation {
    condition = alltrue([
